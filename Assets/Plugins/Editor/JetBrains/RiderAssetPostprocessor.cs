...
namespace Plugins.Editor.JetBrains
{
  public class RiderAssetPostprocessor : AssetPostprocessor
  {
    ...
    public static void OnGeneratedCSProjectFiles()
    {
      ...
      string slnFile = Directory.GetFiles(currentDirectory, "*.sln").First();
    ...
    private static void ApplyManualCompilingSettings(string configFilePath, XElement projectContentElement, XNamespace xmlns)
    {
      if (File.Exists(configFilePath))
      {
        ...
        if (configText.Contains(UNITY_DEFINE_KEYWORD))
        {
          ...
          string[] compileFlags = configText.Split(' ', '\n');
          foreach (string flag in compileFlags)
          {
            string f = flag.Trim();
            if (f.Contains(UNITY_DEFINE_KEYWORD))
            {
              int defineEndPos = f.IndexOf(UNITY_DEFINE_KEYWORD) + UNITY_DEFINE_KEYWORD.Length;
              string definesSubString = f.Substring(defineEndPos,f.Length - defineEndPos);
    ...
    {
      string definesString = string.Join(";", customDefines);

      XElement DefineConstants = projectContentElement
    ...
    private static void FixTargetFrameworkVersion(XElement projectElement, XNamespace xmlns)
    {
      ...
      XElement targetFrameworkVersion = projectElement.Elements(xmlns + "PropertyGroup").
        Elements(xmlns + "TargetFrameworkVersion").FirstOrDefault(); // Processing csproj files, which are not Unity-generated #56
      if (targetFrameworkVersion != null)
      {
        Version version = new Version(targetFrameworkVersion.Value.Substring(1));
    ...
    private static void SetXCodeDllReference(string name, XNamespace xmlns, XElement projectContentElement)
    {
      ...
      string xcodeDllPath = Path.Combine(unityAppBaseFolder, Path.Combine("Data/PlaybackEngines/iOSSupport", name));
      ...
      if (File.Exists(xcodeDllPath))
      {
        ...
        XElement reference = new XElement(xmlns + "Reference");
